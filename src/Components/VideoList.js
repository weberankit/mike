...
const VideoList = () => {
  ...
  return (
    <>
      <h1 className="p-1 font-bold text-xl font-mono  ml-8 sm:ml-12  ">
        LATEST
      </h1>
      <div className=" flex flex-row ">
        {selectOnlyVideos && (
          <button onClick={handleScrollLeft} className="hidden sm:block">
        ...
        <div
          ref={scrollRef}
          className="flex flex-col   sm:flex-row  sm:overflow-x-scroll no-scrollbar"
          onMouseMove={(e) => handleMove(e)}
        >
          {!selectOnlyVideos && (
            <div className="  flex flex-row ">
              {" "}
              <CustomShimmerBox value={5} />
            </div>
          )}
          ...
          {selectSkipVideo &&
            selectOnlyVideos
              .filter((item) => {
                ...
                if (
                  item.id.videoId !== selectSkipVideo.id.videoId &&
              ...
              .map((card, index) => {
                return (
                  <Link
                    to={`/latestVideo/${card.id.videoId}`}
                    key={card.id.videoId}
                    onClick={() => dispatch(addVideoDescData(card))}
                  >
                    <div className="flex  sm:flex-col flex-row justify-center">
                      <div className="  w-1/2 sm:w-96 m-2 rounded-2xl relative">
                        <div className=" xs:hidden   absolute top-1/2 text-white z-40 left-1/3 right-1/2 animate-spin-outline bg-red-500 w-10 rounded-xl m-auto">
                          <Play className="ml-1" size={35} />
                        </div>
                        <img
                          className="sm:hover:bg-black sm:hover:opacity-80 "
                      ...
                      <div className=" w-1/2  sm:w-[80%] p-1   font-semibold  xs:text-sm text-xl sm:text-sm">
                        {" "}
                        {card.snippet.title}
                      </div>
                    </div>
        ...
        {selectOnlyVideos && (
          <button onClick={handleScrollRight} className="hidden sm:block">
