...
const TopView = () => {
  ...
  useEffect(() => {
    ...
    const liveVideo = selectOnlyVideos?.find(
    ...
    liveVideo
      ? dispatch(addVideoSkip(liveVideo))
      : dispatch(addVideoSkip(selectOnlyVideos[0]));
  }, [selectDatavideo, selectOnlyVideos]);

  const selectIframeVideoTopViewStatus = useSelector(
          ...
          <div className="flex flex-col lg:flex-row justify-center lg:justify-between">
            <div className="w-full pt-[20px]  p-2 lg:w-4/6">
              <div className="  video-container">
                {topCard && topCard.snippet.liveBroadcastContent === "live" ? (
                  <span className="absolute bg-red-600 text-white top-0 z-20 p-1 rounded-md">
                    Live video
                  </span>
                ) : (
                  <div className="absolute bg-red-600 text-white top-0 z-20 p-1 rounded-md ">
                    {!slectError && <span>Latest Video</span>}
                    <span className="text-xs">{`${
                      slectError
                        ? "check REDMIKE Youtube  channel for latest videos"
                        : ""
                    }`}</span>
                  </div>
                )}
                {!topCard && <CustomShimmerBox value={1} />}
                {
                  ...
                  selectIframeVideoTopViewStatus && (
                ...
                {topCard && (
                  <YouTube
                    className=" topvideo sm:hover:bg-black sm:hover:opacity-80"
                 ...
                 let timeID= setTimeout(() => {
                          console.log(event.data,"this")
                            if(event.data===2){ console.log("j");clearTimeout(timeID);return};
                          ...
                          setTimeout(() => {
                            ...
                            const allowed = window?.open(
                              `https://www.youtube.com/watch?v=${topCard?.id?.videoId}`,
                              "_blank"
                            );
                        ...
                        if (event.data === 1) {
                            // First timeout to pause the video
                            timeID1 = setTimeout(() => {
                                ...
                                console.log(event.data, "pausing video");
                                event?.target?.pauseVideo();
                                setVideoStop(true);
                            ...
                            }, 10000); // Delay before pausing the video
                        }
              ...
              {topCard && (
                <div>
                  <div className="lg:mt-28">
                    <div className="text-2xl font-extrabold p-2 m-3 lg:mb-4">
                      <h1>{topCard?.snippet?.title}</h1>
                    </div>
                    <div className="bg-red-500   p-2 w-44 ml-6 text-white font-semibold rounded-md lg:ml-8">
                      {" "}
                      <a
                        href={`https://www.youtube.com/watch?v=${topCard?.id?.videoId}`}
                        target="_blank"
                    ...
                    <p className="mt-9 ml-6 ">
                      {topCard?.snippet?.description
                        ?.split(" ")
                        ?.slice(0, 15)
                        ?.join(" ")}
                    </p>
